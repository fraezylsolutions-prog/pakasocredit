                    ...
                    <div class="step-details-form mb-4">
                        <div class="row">
                            ...
                            <div @class([ 'col-xl-4 col-lg-4 col-md-4'=> setting('multiple_currency','permission'),
                                ...
                                <div class="inputs">
                                    <label for="" class="input-label">{{ __('Select Method') }}<span class="required">*</span></label>
<select name="gateway_code" class="box-input deposit-methods" id="gatewaySelect">
    <option data-logo="{{ asset('front/images/payment_gateway.svg') }}" value="" disabled selected>--{{ __('Select Method') }}--</option>
    @if(!setting('multiple_currency','permission'))
    @php
    ...
    <optgroup label="── Manual Deposit ──">
    @foreach ($manualGateways as $gateway)
    <option data-logo="{{ $gateway->logo ? asset($gateway->logo) : asset($gateway->logo ?? 'front/images/payment_gateway.svg') }}" value="{{ $gateway->gateway_code }}">{{ $gateway->name }}</option>
    @endforeach
    </optgroup>
    @endif
    @endif
